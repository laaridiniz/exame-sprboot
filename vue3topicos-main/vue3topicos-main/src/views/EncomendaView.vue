...
// Função para determinar a situação de uma entrega
function situacao(encomenda) {
  if (encomenda.dataHoraEntrega && !encomenda.cancelamento) {
    return "Entregue";
  } else if (encomenda.cancelamento) {
    const dataCancelamento = new Date(encomenda.cancelamento);
    const dataFormatada = dataCancelamento.toLocaleDateString('pt-BR');
    return `Cancelada em ${dataFormatada}`;
  } else {
    return "Pendente";
  }
}
